...
1:3 # quick way to define a sequece in steps of 1

for(i in 1:3) {print(paste(i))}
# can go from 3 to 27 for each 3rd number
...
for(i in 2:length(x)){
  ...
  lines(x[(i-1):i],y[(i-1):i],col='red' )
  }
  else{
    
    lines(x[(i-1):i],y[(i-1):i],col='blue') 
  }
...
dev.new()
setwd('D:/animation') # change the file path to a file path on your computer where you would like the sequence frames (images) to be saved that we will stick together into an animation below
